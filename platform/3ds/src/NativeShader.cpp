	...
	std::unordered_map<Shader::ENamedShaderUniform, std::string> uniformNames) 
{
	...
	_vertexShaderDVLB = std::unique_ptr<DVLB_s>(DVLB_ParseFile((u32*)vshader_shbin, vshader_shbin_size));
	...
	for (auto& [name, uniform] : uniformNames) {
		switch (name) {
			case Shader::ENamedShaderUniform::NormalMatrix: {
				...
				_vertexUniforms.insert_or_assign(name, shaderInstanceGetUniformLocation(&_shaderProgram, "NormalQuat"));
			...
			default: {
				_vertexUniforms.insert_or_assign(name, shaderInstanceGetUniformLocation(&_shaderProgram, uniform.c_str()));
			} break;
		}
	}
	...
	C3D_TexEnvSrc(texEnv, C3D_Both, GPU_FRAGMENT_PRIMARY_COLOR, GPU_FRAGMENT_SECONDARY_COLOR, 0);
